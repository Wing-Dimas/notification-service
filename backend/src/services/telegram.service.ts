...
class TelegramService {
  ...
  public async editMessage(
    ...
    const findMessage = await this.message.findFirst({
      where: { id, notification_type: "TELEGRAM", deleted_at: null },
      include: { message_attachments: true },
    });
    ...
    // const receiver = isNan(body.receiver); // parse receiver to phone number
    const receiver = await this.getReceiver(body.receiver); // parse receiver to chat id
    if (!receiver)
      throw new HttpException(
        409,
        "Message receiver not found, pastikan user sudah terdaftar",
      );
    ...
    return await this.message.update({
      where: { id },
      data: { receiver: receiver.chat_id.toString(), ...data },
    });
  }
  ...
  public async deleteMessage(params: DeleteMessageWADto): Promise<Message> {
    ...
    if (!findMessage) throw new HttpException(409, "Message dosen't exist");
  ...
  public async sendMessage(params: SendMessageWADto): Promise<Message> {
    ...
    try {
      const receiver = await this.getReceiver(payload.receiver);
      if (!receiver)
        throw new HttpException(
          409,
          "Message receiver not found, pastikan user sudah terdaftar",
      ...
      if (!findMessage.message_attachments[0]) {
        await client.sendMessage(receiver.chat_id, payload.message);
      } else {
        // WITH DOCUMENT
